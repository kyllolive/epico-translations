...
const translate = { ...topNav,
  ...
  SCANLATE_BANNER_CONTENT: 'クリエイターがより大きな市場に参入できるよう支援することで、クリエイターをサポートします すべてのコミックに無料でアクセスできます 法的設定でクリエイターと協力できます 検索可能なダイアログ/原稿 チームを変更するための組み込みの用語集',
  SCANLATE_BANNER_HEADER_2: 'あなたの漫画は世界に読まれるに値する',
  SCANLATE_BANNER_CONTENT_2: `漫画のクラウドソーシング翻訳サービスで、あなたの言語が見つかりませんか?私達にメッセージを送ります！`,
  LABEL_CONTENT_HEADER: 'ここで、お気に入りのシリーズをスキャンレートできます。',
  ...
  LABEL_NO_UPLOADED_CLEAN_VERSION: 'アップロードされたクリーン バージョンはありません',
  ...
  LABEL_HOW_TO_COMMENT_3: '[コメントを書く] というテキスト ボックスをクリックし、コメントを入力して、[送信] をクリックします。',
  //Description Tab
  ...
  //Change
  LABEL_CHANGE_PAGE: '保存されていない変更があります。離れてもよろしいですか？変更は失われます'
};
